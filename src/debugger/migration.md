...
|**Debugger**   |               |                |                | 
|Start Process/ Continue execution|`F9`    |`dc` | `F9`             | `run` / `r` and `continue` / `c`     | `g`
|Terminate Process|`Ctrl+F2`    |`dk 9`          | ?                | `kill`          | `q`
|Detach         |`?`            | `o-`           | ?                | `detach`        |
|step into      |`F7`           |`ds`            | `s`              | `step` or `s`   | `t`
|step into 4 instructions | ?   | `ds 4`         | F7               | `step` or `s 4`| `t 4`
|step over      |`F8`           |`dso`           | `S`              | `next` or `n`   | `p` 
|step until a specific address|?|`dsu <addr>`    | ?                | `advance` or `adv` or `until` or `u <addr>` | `g <addr>`
|Run until return|`Ctrl+F7`     |`dcr`           | ?                | `finish` or `f` | `gu`
...
|display Register|On register Windows|`dr all`   | Shown in Visual mode | `info registers` or `i r`    | `r`
|display eax    |On register Windows |`dr?eax`   | Shown in Visual mode | `info registers eax` or `i r eax` or `print $eax` | `r rax`
|display old state of all registers |? |`dro`    | ?                | ?             | ?
|display function addr + N  |?  |`afi $$` - display function information of current offset (`$$`) | ?               | ?             | ?
|display frame state     |?     |`pxw rbp-rsp@rsp`| ?               | `info frame` or `i f` | ?
|How to step until condition is true |?  |`dsi`  | ?                | ?             | ?
|Update a register value |?     |`dr rip=0x456`  | ?                | `set $rip=0x456`             | `r rip=456`
|**Disassembly**   |            |                |                  |
|disassembly forward   | N/A           | `pd`               | `Vp`                 | `disassemble` or `disas` | `uf`, `u`
|disassembly N instructions   | N/A           | `pd X`               | Vp                 | x/<N>i | `u <addr> LX`
|disassembly N (backward)   | N/A           | `pd -X`               | `Vp`                 | `disassemble` or `disas <a-o> <a>` | `ub`
...
|**BackTrace**   |            |                |                  |
|Stack Trace   | N/A           | `dbt`               | N/A                 | `backtrace` or `bt` | `k`
...
|Stacktrace for all threads  | N/A           | `dbt@t`               | N/A                 | `thread apply all backtrace` or `t a a bt` | `~* k`
|**Breakpoints**   |            |                |                  | |
|Breakpoint list |`Ctrl+Alt+B`  |`db`            | ?                | `info breakpoints` or `i br` | `bl`
|add breakpoint |`F2`           |`db [offset]`   |`F2`              | `break` or `b` | `bp`
|**Threads**   |        |                |                  |                  |
|Switch to thread |`Thread menu`           |`dp`   | N/A              | `thread` or `t <N>`| `~<N>s`
|**Frames**   |        |                |                  |                  |
|Frame Numbers |`N/A`           |`?`   | N/A              | `any bt command`| `kn`
|Select Frame |`N/A`           |`?`   | N/A              | `frame` or `f <N>` | `.frame`
|**Parameters/Locals**   |        |                |                  |                  |
|Display parameters |`N/A`           |`afv`   | N/A              | `info args` or `i ar` | `dv /t /i /V`
|Display parameters |`N/A`           |`afv`   | N/A              | `info locals` or `i lo` | `dv /t /i /V`
|Display parameters/locals in json |`N/A`           |`afvj`   | N/A              | N/A | `dv /t /i /V`
